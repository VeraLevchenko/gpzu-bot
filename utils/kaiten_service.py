...
# --- ИСПРАВЛЕНИЕ: BASE_URL теперь использует чистый KAITEN_DOMAIN ---
...
async def create_card(
    ...
    # Используем snake_case для соответствия документации curl
    payload = {
        "board_id": board_id,
        "column_id": column_id,
        "title": title,
        ...
        try:
            async with session.post(url, json=payload, headers=headers) as resp:
                if resp.status in (200, 201):
                    ...
                    card_id = data.get("id")
                    logger.info(f"Карточка создана: ID {card_id} (Board: {board_id}, Column: {column_id})")
                    return card_id
                else:
                    error_text = await resp.text()
                    logger.error(f"Ошибка создания карточки Kaiten: {resp.status} - {error_text}")
                    return None
        ...
        try:
            async with session.post(url, data=form, headers=upload_headers) as resp:
                if resp.status in (200, 201):
                    ...
                    logger.error(f"Ошибка загрузки файла {file_name}: {resp.status} - {text}")
                    return False
        except Exception as e:
