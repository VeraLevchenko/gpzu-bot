...
5. Выводит JSON для пользователя (пока без генерации документа)
"""
...
def _actions_keyboard() -> InlineKeyboardBuilder:
    ...
    kb.button(text="📊 Показать JSON", callback_data="gp:show_json")
...
@gp_router.callback_query(GPStates.SHOW_RESULTS, F.data == "gp:show_json")
async def gp_show_json_handler(call: CallbackQuery, state: FSMContext):
    """
    Показать полный JSON пользователю
    """
    await call.answer()
    ...
    # Отправляем JSON красиво отформатированным
    try:
        # Парсим и форматируем с отступами
        json_obj = json.loads(gp_json)
        formatted_json = json.dumps(json_obj, ensure_ascii=False, indent=2)
        
        # Telegram ограничивает длину сообщения 4096 символами
        # Если JSON большой, разбиваем на части
        max_chunk = 4000  # оставляем запас для форматирования
        
        if len(formatted_json) <= max_chunk:
            await call.message.answer(
                f"📊 *Полные данные для ГП (JSON):*\n\n"
                f"```json\n{formatted_json}\n```",
                parse_mode="Markdown",
            )
        else:
            ...
            for i, chunk in enumerate(chunks, 1):
                header = f"📊 *Данные для ГП (часть {i}/{len(chunks)}):*\n\n" if len(chunks) > 1 else "📊 *Полные данные для ГП (JSON):*\n\n"
                await call.message.answer(
                    f"{header}```json\n{chunk}\n```",
                    parse_mode="Markdown",
                )
        ...
        await call.message.answer(
            "✅ Это полная структура данных, собранная для градплана.\n\n"
            "Используйте кнопки выше для дальнейших действий.",
        )
        
    except Exception as ex:
        logger.exception(f"Ошибка при выводе JSON: {ex}")
        await call.message.answer(
            f"❌ Ошибка при форматировании данных:\n{ex}\n\n"
            f"Сырой JSON (первые 500 символов):\n```\n{gp_json[:500]}...\n```",
            parse_mode="Markdown",
        )
...
async def gp_generate_handler(call: CallbackQuery, state: FSMContext):
    ...
    Пока выводим краткую информацию.
    """
    await call.answer()
    ...
    await call.message.answer(
        ...
        "✅ Все данные собраны и готовы для генерации:\n"
        "• Данные заявления\n"
        "• Информация о земельном участке с координатами\n"
        ...
        "1. Создание шаблона документа ГП\n"
        "2. Генератор DOCX на основе собранных данных\n"
        "3. Автоматическая отправка готового документа\n\n"
        "💡 Используйте кнопку \"📊 Показать JSON\" чтобы увидеть все собранные данные.",
        parse_mode="Markdown",
    )
